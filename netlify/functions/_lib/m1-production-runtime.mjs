...
function profileProductionOrigin(options = {}) {
  if (options.staffClock !== true) return PRODUCTION_ORIGIN;
  const profile = deploymentInstallationProfile(
    options.installationId,
    options.environment,
    options.activation
  );
  const origin = exactString(profile?.allowedOrigin);
  if (profile?.featureFlags?.staffClock !== true || !origin) return '';
  try {
...
export function validExactProductionRequest(request, path, options = {}) {
  const requiredOrigin = profileProductionOrigin(options);
  if (!requiredOrigin) return false;
  const expected = new URL(requiredOrigin);
  let url;
  try {
  ...
  return typeof host === 'string'
    ...
    && request.headers.get('origin') === requiredOrigin
...
export function productionRuntimeConfig(env = process.env, options = {}) {
  ...
  const pairingMode = options.staffClockPairing === true;
  const staffClockMode = options.staffClock === true;
  const pairing = profile?.staffClockPairing;
  const requiredOrigin = pairingMode
    && profile?.featureFlags?.staffClockPairing === true
    && pairing?.origin === profile.allowedOrigin
    ? pairing.origin
    : pairingMode
      ? ''
      : staffClockMode && profile?.featureFlags?.staffClock === true
        ? profile.allowedOrigin
        : staffClockMode
          ? ''
          : PRODUCTION_ORIGIN;
  if (
    !requiredOrigin
    || (!pairingMode && !staffClockMode && profile?.installationId !== 'rev')
  ) return null;
...
export function productionAdminInstallerConfig(env = process.env, options = {}) {
  const runtime = productionRuntimeConfig(env, options);
  const installSecret = validSecret(env.GIB_M1_PRODUCTION_INSTALL_CAPABILITY_SECRET);
  if (
    !runtime
    || !installSecret

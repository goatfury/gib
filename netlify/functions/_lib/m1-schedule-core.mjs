...
export const MAX_CLASS_LABEL_LENGTH = 200;
...
const MAX_TOTAL_GROWTH_RATIO = 1.3;
const MAX_DAILY_GROWTH_RATIO = 0.5;
const MIN_GLOBAL_RETAINED_RATIO = 0.7;
const MIN_DAILY_RETAINED_RATIO = 0.5;
...
function retainedLabelCount(nextLabels, previousLabels) {
...
export function validatePlausibleScheduleTransition(nextDays, previousDays) {
  ...
  if (nextTotal > Math.ceil(previousTotal * MAX_TOTAL_GROWTH_RATIO)) {
    fail('unexpected-schedule-growth');
  }
  for (const day of SCHEDULE_DAYS) {
    const previousCount = previous[day].length;
    const allowedGrowth = Math.max(2, Math.ceil(previousCount * MAX_DAILY_GROWTH_RATIO));
    if (next[day].length - previousCount > allowedGrowth) {
      fail('unexpected-schedule-growth');
    }
  }

  const retainedTotal = retainedLabelCount(Object.values(next).flat(), Object.values(previous).flat());
  if (retainedTotal < Math.ceil(previousTotal * MIN_GLOBAL_RETAINED_RATIO)) {
    fail('unexpected-schedule-churn');
  }
  for (const day of SCHEDULE_DAYS) {
    const previousCount = previous[day].length;
    if (
      retainedLabelCount(next[day], previous[day])
        < Math.ceil(previousCount * MIN_DAILY_RETAINED_RATIO)
    ) {
      fail('unexpected-schedule-churn');
    }
  }

...
- [ ] Suspend/background the page across each deadline, then resume or navigate back. Expired confirmation details clear on return even when the normal timer did not run. Exercise actual browser Back as well as lifecycle signals: native form-history restoration must not bring the completed name, notes, class selections, or Staff selection back after page initialization. Confirm every class checkbox remains unselected, including regular, local temporary, and shared added classes. Do not interpret browser lifecycle emulation as a physical-tablet sleep pass.

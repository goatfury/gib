  ...
  return {
    ...
    source: {
      ...
      modifiedAt: overrides.modifiedAt || (sourceType === 'wordpress-rest'
        ? '2026-08-09T11:58:26Z'
        : '2026-08-09')
    },
    fetchedAt: overrides.fetchedAt || '2026-08-09T13:00:00.000Z',
    current,
    fallback,
    status: {
      ...
      servedAt: overrides.servedAt || '2026-08-09T13:00:01.000Z'
    },
...
function kioskValidationRuntime(extras = {}) {
  ...
  new vm.Script(`
    ...
    const CHECKED_IN_SCHEDULE_VERSION = '2026-08-09';
...
function kioskRefreshRuntime({ storage, fetchImpl, override = null }) {
  const refresh = sourceBetween(kiosk, 'async function refreshCanonicalSchedule()', '// Sign-ins');
...
test('kiosk bootstrap and Daily Review share one checked-in Rev schedule contract', () => {
  ...
  assert.equal(Object.values(shared.days).flat().length, 54);
...
test('website schedule, validated offline cache, and built-in bootstrap have explicit precedence', () => {
  ...
  assert.match(refresh, /refreshScheduleViews\(\{ preserveSelections: true \}\)/u);
});
...
test('executed kiosk precedence visibly distinguishes shared, manual, URL, and disabled modes', () => {
  ...
  for (const source of ['manual', 'url', 'disabled']) {
    ...
    assert.match(status, /Local (?:manual|URL|disabled) schedule override/u, source);
    assert.match(status, /Website schedule updates are paused on this device/u, source);
    assert.match(status, /saved/u, source);
  }
});
...
test('Series remains a separate chronological overlay and delayed refresh preserves selections', () => {
  const today = sourceBetween(kiosk, 'function classesForToday()', 'function populateClassesForToday(');
  assert.match(today, /sortClassesChronologically\(mergeUniqueClasses\(scheduled, series\)\)/u);

  const populate = sourceBetween(kiosk, 'function populateClassesForToday(', 'function refreshScheduleViews(');
  assert.match(populate, /options\.preserveSelections && selected\.size/u);
  assert.match(populate, /scheduleViewRefreshPending = true/u);
  assert.match(kiosk, /if \(scheduleViewRefreshPending && !selectedClasses\(\)\.length\)/u);
  assert.ok((kiosk.match(/applyPendingScheduleViewIfSafe\(\)/gu) || []).length >= 3);
});
...
test('both screens visibly expose canonical source, version, and fallback state', () => {
  ...
  assert.match(kiosk, /Website schedule .* current .*\$\{version\}/u);
  assert.match(kiosk, /fallback cache/u);

...
function googleReview(body = original) {
...
function request(body = original, options = {}) {
  ...
  return new Request(options.url || origin + config.path, { method: options.method || 'POST',
    headers: { 'Content-Type': 'application/json', Origin: origin, Cookie: `${ADMIN_COOKIE}=${encodeURIComponent(cookie)}`,
...
async function run(body = original, review = googleReview(body), overrides = {}, options = {}) {
  const calls = [];
  const response = await handleAdminAddCheck(request(body, options), { ...dependencies, fetch: async (_url, init) => {
...
test('the actual receiver addition and Daily Review contract confirm read-only with two legitimate instructors', async () => {
  ...
  const sheet = initial => {
    ...
    return { rows, appendRow: row => rows.push([...row]), getDataRange: () => ({ getValues: () => structuredClone(rows) }), getLastRow: () => rows.length,
  ...
  const spreadsheet = { getName: () => 'TEST ONLY synthetic integration', getSheetByName: name => sheets.get(name),
  ...
  const context = vm.createContext({
    ...
    EXPECTED_SPREADSHEET_NAME: 'TEST ONLY synthetic integration', TEST_SPREADSHEET_ID: 'synthetic-test-sheet', GIB_M1_ALLOWED_TARGET: 'test',
    ContentService: { MimeType: { JSON: 'application/json' }, createTextOutput: text => ({ text, setMimeType() { return this; } }) },
    LockService: { getScriptLock: () => ({ tryLock: () => true, releaseLock() {} }) },
    PropertiesService: { getScriptProperties: () => ({ getProperty: name => ({ GIB_M1_RECEIVER_TRANSPORT_TOKEN: env.GIB_TEST_WEBHOOK_TOKEN, GIB_M1_ADMIN_ACTION_TOKEN: env.GIB_TEST_ADMIN_ACTION_TOKEN })[name] || '' }) },
    SpreadsheetApp: { openById: () => spreadsheet, flush() {} }, Utilities: { formatDate }
  });
  ...
  const envelope = { token: env.GIB_TEST_WEBHOOK_TOKEN, adminActionToken: env.GIB_TEST_ADMIN_ACTION_TOKEN, target: 'test', adminName };
...
test('complete Daily Review envelope validation is required and the new endpoint contains no write action', () => {
  ...
  assert.ok(additionReceiptFromDailyReview(daily, original, adminName));
  for (const patch of [{ test: false }, { adminName: 'Andrew Smith' }, { extra: true }, { ok: false }]) {
    assert.equal(additionReceiptFromDailyReview({ ...daily, ...patch }, original, adminName), null);
  }
